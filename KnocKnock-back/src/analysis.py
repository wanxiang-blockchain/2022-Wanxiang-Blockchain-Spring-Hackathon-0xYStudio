import json
import getDataFromDB as getDapps
...
def main():
    ...
    # get data from DB
    dapp_json = getDapps.scan_dynamodb('Stats')
    ...
    dapp_df_from_S3 = spark.read.json(
        "dapp_stats.json", schema=dapp_stats_schema)
    dapp_df_from_S3.createOrReplaceTempView("dapp_df_from_S3")

    dapp_volume_view = spark.sql(
        "select slug as dapp_name,round(d.one_day_volume,4) as one_day_volume,round(d.seven_day_volume,4) as seven_day_volume,round(d.thirty_day_volume,4) as thirty_day_volume FROM  dapp_df_from_S3 d order by total_sales desc")
    dapp_volume_view.createOrReplaceTempView("dapp_volume_view")

    # Trend 1 : Storing Dapp daily,weekly and monthly volumes
    dapp_top10_volume_view = spark.sql(
        "select *  from dapp_volume_view limit 10")
    dapp_top10_volume_view.createOrReplaceTempView("dapp_top10_volume_view")
    dapp_top10_volume_view.coalesce(1).write.mode(
        "overwrite").json("dapp_volume.json")

    dapp_ratio_view = spark.sql(
        "select slug as dapp_name,round(d.market_cap,4) as market_cap,round(d.total_volume,4) as total_volume, round(d.market_cap,4)/round(d.total_volume,4) as ratio FROM  dapp_df_from_S3 d order by total_sales desc")
    dapp_market_cap_to_volume_ratio = dapp_ratio_view.withColumn("ratio", when(
        (dapp_ratio_view.ratio < 1), lit("Sub-Optimal")).otherwise(lit("Optimal")))
    dapp_market_cap_to_volume_ratio.createOrReplaceTempView(
        "dapp_market_cap_to_volume_ratio")

    # Trend 2 :Determinign Dapp Optimality and Storing
    dapp_optimality = spark.sql(
        "select dapp_name,ratio from dapp_market_cap_to_volume_ratio")
    dapp_optimality.createOrReplaceTempView("dapp_optimality")
    dapp_optimality.coalesce(1).write.mode(
        "overwrite").json("dapp_optimality.json")
